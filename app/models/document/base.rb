...
class Document::Base < ActiveRecord::Base
  ...
  def is_editable?(user); 
    ...
    return false if self.signee_motions.where(receiver_user: user).any? && user.id != AUTO_SIGNEE
      ...
      motions.each do |m|
        if m["deleted"]
          ...
          if m["receiver_type"] == 'hr.Employee'
            ...
            # docuser.delete if ( docuser.present? && self.users.where(user: receiver_user).count == 1 )
          end
        end
        if m["temp"]
          motion_params = {
