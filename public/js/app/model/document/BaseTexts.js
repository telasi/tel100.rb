Ext.define('Telasi.model.document.BaseTexts',{
    alias: 'app.fieldnames',
    singleton: true,

    text_DocNumberField:        'შიდა #',
    text_DocDateField:          'დოკუმენტის თარიღი',
    text_DocDateField2:         'თარიღი',
    text_TypeField:             'ტიპი',
    text_OriginalNumberField:   'გარე #',
    text_OriginalDateField:     'გარე თარიღი',
    text_SubjectField:          'თემა',
    text_DeadlineField:         'ვადა',
    text_AlarmField:            'კონტროლი',
    text_PagecountField:        'გვერდები',
    text_AttachmentsField:      'დანართები',
    text_DirectionField:        'მიმართულება',
    text_StatusField:           'სტატუსი',

    text_attachmentField:       '',
    text_actionField:           'მოქმედება',
    text_AnswersField:          'პასუხები',
    text_RecieversField:        'ადრესატები',
});
